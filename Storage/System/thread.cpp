	...
	RC thread_t::runTest(txn_man * txn)
	{
		...
		if (g_test_case == READ_WRITE) {
			...
			if (rc == RCOK)
				...
				return rc;
		} else if (g_test_case == TREE_WINNER_OVERRIDE
			|| g_test_case == TREE_SIBLING_ISOLATION
			|| g_test_case == TREE_EXPLORATORY_ROOT_READ
			|| g_test_case == TREE_WINNER_REFRESH_UPDATE
			|| g_test_case == TREE_CONFLICT_ABSENT_INSERT
			|| g_test_case == TREE_ABORT_CLEANUP) {
			rc = ((TestTxnMan *)txn)->run_txn(g_test_case, 0);
			if (rc == RCOK) {
