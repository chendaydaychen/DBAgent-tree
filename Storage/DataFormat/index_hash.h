...
namespace storage
{
	...
	public:
		...
		BucketHeader * locate_bucket(idx_key_t key, int part_id = 0);
		void latch_bucket(BucketHeader * bucket);
		void unlatch_bucket(BucketHeader * bucket);
		bool bucket_contains_exact_key(BucketHeader * bucket, idx_key_t key, const std::string &key_str, const char * tname);
	private:
